...
SignalOps AI is evaluated against 17 deterministic campaign troubleshooting cases spanning pacing, narrow targeting, creative rejection, VAST errors, bid floors, frequency caps, delayed launch, device mismatch, shared inventory pressure, bid competitiveness (campaign 1050), and launch timing (campaign 1051).
...
| Backend tests | 87 passing | 100% passing |
| Query-intent differentiation | 3/3 focused lenses | 3/3 |
| Unsupported root-cause tolerance | 0 | 0 |

## Decision Gate Evaluation (Phase 2)

See [`docs/evals/`](./evals/) for dated reports from `python -m evals.gate_evaluation` (manual-only, real network calls when a provider key is configured, never run in CI). The most recent run (2026-09-24, no `OPENAI_API_KEY`/`TYPESAFE_API_KEY` configured in that session):

| Metric | JevGate | LLMGate | RuleGate |
|---|---|---|---|
| Accuracy | NOT RUN | NOT RUN | 100%* |
| Schema/type error rate | NOT RUN | NOT RUN | 0% |

\* RuleGate's 100% includes case A08, which is scored against RuleGate's own documented, known-wrong answer for unsupported-partner-blame detection (a semantic judgment RuleGate structurally cannot make from keywords alone) — see `docs/evals/2026-09-24-gate-report.md` and `backend/evals/adversarial_cases.json` for the honest caveat. JevGate/LLMGate live comparison numbers will be added once credentials are configured and the command is re-run — never fabricated in the meantime.
...
| Late campaign start | 1048, 1051 | G12, G17 |
| Shared high-priority inventory pressure | 1049 | G15 |
| Bid competitiveness | 1047, 1050 | G07, G08, G16 |
